...
app.post('/reminders', async (c) => {
  ...
  if (!isValidDate(dueDate)) {
    return c.json({ error: 'Bad Request: Invalid date format.' }, 400);
  }
...
// GET /reminders/due-in-x-days - Retrieve all reminders due in 'x' days (dynamic)
app.get('/reminders/due-in/:days', (c) => {
  const days = parseInt(c.req.param('days'), 10);
  const dueInXDays = reminders.filter((r) => isBefore(parseISO(r.dueDate), addDays(new Date(), days)));

  if (dueInXDays.length === 0) {
    return c.json({ error: `Not Found: No reminders due in the next ${days} days.` }, 404);
  }

  return c.json(dueInXDays, 200);
});
